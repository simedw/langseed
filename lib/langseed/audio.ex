defmodule Langseed.Audio do
  ...
  defp strip_emojis(text) do
    # Remove emoji and emoji component characters using Unicode ranges
    # Covers most common emoji ranges (emoticons, symbols, pictographs, transport, etc.)
    text
    |> String.replace(~r/[\x{1F600}-\x{1F64F}\x{1F300}-\x{1F5FF}\x{1F680}-\x{1F6FF}\x{1F1E0}-\x{1F1FF}\x{2600}-\x{26FF}\x{2700}-\x{27BF}\x{FE00}-\x{FE0F}\x{1F900}-\x{1F9FF}\x{1FA00}-\x{1FA6F}\x{1FA70}-\x{1FAFF}]/u, "")
    |> String.replace(~r/\s+/, " ")
    |> String.trim()
  end
